...
int main(int argc, char* argv[]) {
    ...
    std::vector<stupid_integral::integral_thread_t> it_data(thr_num);
    
    for (int i = 0; i < thr_num; ++i) {
        it_data[i].tid = i;
        it_data[i].res_vec = &res_vec;
        it_data[i].start  = stupid_integral::get_bounder(start, finish, i - 1, thr_num);;
        it_data[i].finish = stupid_integral::get_bounder(start, finish, i, thr_num);
        it_data[i].n_steps = n_steps / thr_num;
    }

    for (int i = 0; i < thr_num; ++i) {
        int ret_code = pthread_create(&thr[i], NULL, stupid_integral::thread_integral, &it_data[i]);
